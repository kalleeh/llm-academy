...
export const SelfExplain: React.FC<SelfExplainProps> = ({ prompt, modelAnswer }) => {
  ...
  return (
    <div className="border-l-2 border-amber-500/30 bg-zinc-900/50 pl-4">
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="flex w-full items-center gap-2 py-3 text-left text-sm font-medium text-amber-400/80 transition-colors hover:text-amber-300"
      >
        <Icon
          name="lightbulb"
      ...
      {!collapsed && (
        <div className="space-y-3 pb-4">
          <p className="text-sm text-zinc-300">{tr.prompt}</p>
          <textarea
            value={text}
          ...
          {revealed && (
            <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4">
              <p className="mb-2 text-xs font-medium text-emerald-400">{t(lang, 'check.modelAnswer')}</p>
              <p className="text-sm text-zinc-300">{tr.modelAnswer}</p>
            </div>
          )}
        </div>
      )}
