  ...
  return (
    <li role="treeitem" aria-expanded={node.type === 'folder' ? expanded : undefined}>
      ...
      >
        ...
        <span className={node.type === 'folder' ? 'font-medium text-zinc-200' : ''}>
          {node.name}
        </span>
        {node.size && <span className="ml-auto text-xs text-zinc-500">{node.size}</span>}
        {node.annotation && (
          <span className="ml-2 text-xs text-amber-400/80 italic">{node.annotation}</span>
        )}
      </button>
...
export const FileExplorer: React.FC<FileExplorerProps> = ({ tree, title }) => {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-700">
      ...
      <div className="flex">
        <div className="min-w-0 flex-1 bg-zinc-900 py-2">
        ...
        {selectedFile?.content && (
          <div className="w-1/2 border-l border-zinc-700 bg-zinc-950 p-4">
