  ...
  modules: {
    ...
    toolslandscape: {
      ...
      agenticLoop: {
        ...
        intro: '에이전틱 코딩 도구에 작업을 맡기면 실제로 무슨 일이 일어날까요? 아래에서 같은 버그 수정이 두 개의 실제 CLI에서 실행됩니다 — Claude Code와 Kiro를 전환하며 각각이 어떻게 작동하는지 비교해 보세요.',
        stepNote: '각 턴을 단계별로 살펴보세요. Claude Code는 직접적인 도구 루프로 작동합니다 — 검색, 읽기, 편집, 테스트. Kiro는 스펙 우선으로 작동하여 코드를 바꾸기 전에 요구사항, 설계, 작업을 생성합니다. 같은 수정, 다른 방식.',
      ...
      pickingTools: {
        ...
        recommendLabel: '최적의 선택:',
        scenarios: [
          { situation: '한 클리닉의 안내 데스크가 매일 오후를 예약 시스템, 취소 이메일, 손으로 쓴 메모로 다음 날 예약 목록을 정리하는 데 씁니다.', pick: '에이전틱 업무 앱', why: '반복적이고, 출처가 여럿이고, 결과물이 명확함 — 이상적인 위임입니다. 에이전트가 출처들로 목록을 정리하고 직원이 몇 분 만에 검토합니다. 챗 어시스턴트라면 매일 모든 것을 수동으로 붙여넣어야 합니다.' },
          { situation: '한 학교 사무실이 현장학습이 승인되기 전에 들어오는 모든 동의서를 교육청 정책과 대조해 확인해야 합니다.', pick: '에이전틱 업무 앱 — 필수 인간 검토와 함께', why: '에이전트가 각 동의서를 정책과 비교하고 누락되거나 규정에 맞지 않는 것을 근거와 함께 표시합니다. 최종 판단은 직원이 합니다. 고위험 확인은 사람을 의사결정자로 남기고, 에이전트는 읽는 시간을 없애줍니다.' },
          { situation: '한 식당 주인이 새 계절 메뉴의 메뉴 설명과 소셜 게시물 몇 개를 다듬는 데 도움을 원합니다.', pick: '챗 어시스턴트', why: '창의적 반복은 대화입니다 — 옵션 생성, 반응, 다듬기. 다룰 파일도, 다단계 워크플로우도 없습니다. 작동하는 가장 단순한 도구가 옳은 도구입니다.' },
          { situation: '한 재무 백오피스가 매월 수백 건의 공급업체 송장을 구매 주문과 대조하느라 팀이 그 작업에 잡아먹히고 있습니다.', pick: '에이전틱 업무 앱 (그리고 반복된다면 맞춤 제작 도구)', why: '반복적이고, 구조화되어 있고, 검증 가능함 — 이상적인 에이전트 작업이며, 작은 재사용 가능 도구로 발전시킬 강력한 후보입니다. 에이전트가 송장을 주문과 매칭하고 불일치를 표시하면, 사람이 예외를 검토합니다.' },
        ],
        selfExplainPrompt: '팀이 매주 하는 가장 반복적인 다단계 작업을 떠올려 보세요. 에이전틱 도구에 위임할 수 있을까요? 도구에 줄 한 단락짜리 브리프를 써 보세요.',
        selfExplainAnswer: '예시: "매주 월요일 우리 클리닉은 그 주 예약에서 노쇼와 후속 조치 목록을 정리합니다. 브리프: 지난주 예약 내보내기를 스캔, 진료 제공자별로 그룹화해 모든 누락된 예약을 나열, 어느 것이 후속 전화가 필요한지 표시, 맨 위에 두 줄 요약; 긴급으로 표시된 것은 모두 플래그. 한 페이지로 출력." 명확한 출처, 형식, 에스컬레이션 규칙 — 그것이 위임 준비가 된 브리프이며, 매주 반복된다면 맞춤 제작을 기다리는 도구이기도 합니다.',
      },
    },
    // MT
    workingwithai: {
      ...
      vagueToValuable: {
        ...
        stepLabel: '단계',
        appToggleLabel: '어시스턴트 선택: ChatGPT 또는 Claude',
        steps: [
      ...
      agentContext: {
        title: '4. 컨텍스트를 부여하세요',
        introBusiness:
          '범용 어시스턴트는 일반적인 것은 다 알지만 당신에 대해서는 아무것도 모릅니다. 가장 큰 업그레이드는 더 영리한 프롬프트가 아니라, 회사·프로젝트·당신의 역할에 대한 상시 컨텍스트를 모델에 주어 더 이상 낯선 사람처럼 답하지 않게 하는 것입니다. 아래에서는 같은 요청을 두 가지로 실행합니다: 일반 어시스턴트와, 위 컨텍스트 카드로 브리핑된 어시스턴트입니다. 탭을 전환하며 차이를 확인하세요.',
        introTechnical:
          '새 에이전트는 언어는 알지만 당신의 코드베이스는 전혀 모릅니다. 가장 큰 업그레이드는 더 영리한 프롬프트가 아니라, 회사·프로젝트·따라야 할 컨벤션을 에이전트에 알려주는 상시 컨텍스트(AGENTS.md / CLAUDE.md / 스티어링 파일)입니다. 아래에서는 같은 작업을 두 가지로 실행합니다: 맨몸 에이전트와, 위 컨텍스트 카드를 읽은 에이전트입니다. 탭을 전환하며 차이를 확인하세요.',
        cardTitle: '상시 컨텍스트 (한 번 설정하면 매 세션 로드됨)',
        company: {
          label: '회사',
          body: 'Bright Smile Dental — 동네 치과. 따뜻하고 쉬운 말, 전문 용어 없음. 항상 구체적인 두 개의 시간을 제안하고 환자에게 확인을 요청합니다.',
        },
        project: {
          label: '프로젝트',
          body: '이번 분기 노쇼 줄이기. 목표: 모든 일정 변경 답장이 "편한 시간을 알려주세요" 대신 구체적인 빈 시간 두 개와 원터치 확인 방법을 제시한다.',
        },
        roleBusiness: {
          label: '나',
          body: '프런트 데스크 책임자. 톤은 환자에게, 꽉 찬 예약표는 치과의사에게 책임집니다. 다시 써야 하는 양식 편지가 아니라, 짧고 그대로 보낼 수 있는 답장을 원합니다.',
        },
        roleTechnical: {
          label: '컨벤션',
          body: 'Express가 아니라 TypeScript + Fastify. 시간은 라이브 캘린더에서 가져옴 — 하드코딩 금지. 오류는 래핑하고 잘못된 입력은 400. 모든 엔드포인트는 테이블 기반 테스트와 함께 제공.',
        },
        taskLabel: '요청 (두 탭에서 동일):',
        taskBusiness: '"화요일 스케일링을 옮겨달라고 한 Sara에게 답장해 주세요."',
        taskTechnical: '특정 날짜의 빈 예약 시간을 반환하는 엔드포인트를 추가해 줘',
        genericTab: '일반 어시스턴트',
        briefedTab: '컨텍스트로 브리핑됨',
        toggleLabel: '일반 어시스턴트와 브리핑된 어시스턴트 전환',
        genericSteps: [
          { label: '나', content: '화요일 스케일링을 옮겨달라고 한 Sara에게 답장해 주세요.' },
          {
            label: '어시스턴트',
            content:
              '초안입니다: "환자분께, 예약 관련해 연락 주셔서 감사합니다. 요청을 기꺼이 도와드리겠습니다. 빈 시간이 있습니다. 원하시는 날짜와 시간을 알려주시면 최선을 다해 도와드리겠습니다. 연락 기다리겠습니다."',
            note: '병원도, 캘린더도, 누가 읽는지도 모릅니다. 그래서 미사여구로 채우고 양식 편지처럼 들리며, 예약을 진전시키는 대신 일을 Sara에게 다시 떠넘깁니다 — "편한 시간을 알려주세요".',
          },
        ],
        briefedSteps: [
          { label: '나', content: '화요일 스케일링을 옮겨달라고 한 Sara에게 답장해 주세요.' },
          {
            label: 'Claude',
            content:
              '"안녕하세요 Sara님 — 스케일링 일정 기꺼이 옮겨드릴게요! 다음 주 화요일 오전 9시 40분 또는 목요일 오후 2시가 비어 있습니다. 편하신 시간으로 답장 주시면 바로 잡아드릴게요. — Bright Smile Dental 프런트 데스크"',
            note: '같은 모델, 같은 요청. 컨텍스트 카드가 병원, 따뜻하고 쉬운 톤, 두 개의 시간을 제안하는 규칙, 그리고 그대로 보낼 수 있는 답장이 필요하다는 점을 알려주었기에 — 정확히 그것을, 다시 쓸 필요 없이 만들어냈습니다.',
          },
        ],
        bridgeBusiness:
          '브리핑된 실행에서 더 나았던 것은 프롬프트가 아니라 더 나은 상시 컨텍스트였습니다. 모든 채팅에서 참인 회사·프로젝트·역할 사실은 매번 다시 쓰는 게 아니라 사용자 지정 지침에 들어가야 합니다. 아래 스타터 키트가 바로 그것을 설정합니다.',
        bridgeTechnical:
          '두 번째 실행에서 더 나았던 것은 프롬프트가 아니라 에이전트가 먼저 읽은 AGENTS.md였습니다. 모든 작업에 걸쳐 적용되는 컨벤션은 각 프롬프트가 아니라 에이전트가 자동으로 로드하는 컨텍스트 파일에 들어가야 합니다. 아래 스타터 키트가 바로 그것을 설정합니다.',
        selfExplainPrompt:
          '세 가지 컨텍스트 블록 — 회사, 프로젝트, 역할/컨벤션 — 중 당신이 실제로 하는 일에서 답을 가장 크게 바꿀 것은 무엇이며, 그 이유는?',
        selfExplainAnswer:
          '정답은 하나가 아닙니다 — 핵심은 각 블록이 서로 다른 종류의 추측을 제거한다는 점입니다. 회사/컨벤션 컨텍스트는 결과물이 어떻게 보이고 들려야 하는지(톤, 기술 스택, 사내 규칙)를 고정해 매번 같은 것을 고치지 않게 합니다. 프로젝트 컨텍스트는 그 작업이 무엇을 위한 것이고 무엇을 위반하면 안 되는지(목표, 제약, 이해관계자)를 고정해, 기술적으로는 맞지만 핵심을 놓친 답을 막아줍니다. 역할 컨텍스트는 결과물이 누구를 위한 것이고 어떤 형태여야 하는지를 고정해, 다시 다듬을 필요 없이 바로 실행할 수 있는 것을 줍니다. 가장 자주 다시 입력하는 것이 가장 먼저 상시 컨텍스트로 승격할 가치가 있습니다 — 반복되는 그 문장이 신호입니다.',
      },
      starterKit: {
        title: '5. 한 번만 설정하기: 나의 스타터 키트',
        intro:
          '이 모듈이 가르치는 모든 것은 다시 입력하지 않아도 작동합니다. 컨텍스트는 한 번만 들어갑니다 — 모든 프롬프트가 아니라 설정에 말이죠. 이 두 가지 템플릿을 복사하고, 괄호를 채우면, 5분 만에 준비가 끝납니다.',
        templateTitleA: '어시스턴트의 사용자 지정 지침에 붙여넣기',
        templateTitleB: '이 골격을 저장하세요 — 매번 괄호를 채우세요',
        whereTitle: '설정이 있는 위치',
        where: [
        ...
        tips: [
          {
            name: '나를 인터뷰하게 만들기',
            body: '큰 요청은 "답하기 전에 명확히 하기 위한 질문을 최대 3개까지 나에게 해줘"로 끝맺으세요. 한 문장이면 일반적인 결과 대신 맞춤형 결과를 얻습니다.',
          },
          {
            name: '바꿔 말하지 말고 붙여넣기',
            body: '문서에 대한 당신의 요약은 손실 있는 사본입니다. 실물을 첨부하거나 붙여넣으세요 — 모델은 당신이 설명하는 것보다 빠르게 읽습니다.',
          },
          {
            name: '여러 버전을 요청하고, 고른 뒤, 다듬기',
            body: '"안전한 것, 과감한 것, 짧은 것 3가지 버전을 줘"가 재생성 룰렛보다 낫습니다. 하나를 골라 "2번 같은데, 다만…"이라고 말하세요.',
          },
          {
            name: '무엇을 뺄지 말해주기',
            body: '"이모지 금지, 느낌표 금지, 내 질문을 다시 말하지 마"는 가장 흔한 짜증거리를 한 줄로 없앱니다 — 그리고 이는 모든 프롬프트가 아니라 사용자 지정 지침에 들어가야 합니다.',
          },
          {
            name: '같은 작업에는 스레드 재사용하기',
            body: '후속 질문은 당신이 설정한 모든 것을 상속합니다. 다만 주제를 바꿀 때는 새 채팅에서 하세요, 그러지 않으면 옛 컨텍스트가 새 답변에 스며듭니다.',
          },
        ],
        takeaway:
          '5분의 설정은 이미 당신을 아는 어시스턴트와 매일 처음부터 다시 브리핑해야 하는 낯선 사람 사이의 차이입니다.',
      },
    },
    // MT
    optimizingworkflow: {
      ...
      oneOffToSystem: {
        ...
        appToggleLabel: '어시스턴트 선택: ChatGPT 또는 Claude',
        steps: [
          { label: '주간 고역', content: '매주 월요일 한 학교 사무실이 학부모를 위한 같은 출석 및 행사 요약을 다시 만듭니다: 스프레드시트 세 개를 열고, 하이라이트를 채팅에 붙여넣고, 형식을 다시 설명하고, 어조를 고치고, 소식지에 맞게 재포맷합니다. 90분, 매주, 처음부터.', note: '매번 셋업 비용을 전부 다시 치르고 있습니다. 그것이 낭비입니다.' },
        ...
        selfExplainAnswer: '예시: "월간 가족 소식지. 저장된 브리프: 컨텍스트(누가 읽고 무엇에 관심 있는지), 세 섹션 형식, 따뜻하지만 간결한 어조. 프로젝트 내용물: 행사 캘린더 내보내기, 연속성을 위한 지난달 소식지, 학교 보이스 메모. 그러면 \'이번 달 소식지 초안 작성\'이 필요한 모든 것에서 출발합니다." 그리고 그 안의 한 단계가 매달 똑같다면, 그 단계는 작은 도구로 맞춤 제작할 후보입니다.',
      },
      rollItOut: {
        title: '3. 팀에 확산하기',
      ...
      // MT
      promptLibrary: {
        title: '4. 당신의 첫 프롬프트 라이브러리',
        intro:
          '프롬프트 라이브러리는 거창하게 들리지만, 메모 문서일 뿐입니다. 이 세 가지로 시작하세요 — 각각 한 번 쓰고, 매주 재사용하세요. 복사하고, 괄호를 채우고, 채운 버전을 저장하세요.',
        templateTitleA: '주간 현황 보고서',
        templateTitleB: '회의 → 결정 사항과 실행 항목',
        templateTitleC: '나의 어조로 쓴 고객 답변',
        tipsTitle: '프로 팁',
        tips: [
          {
            name: '작동하는 순간 바로 저장하기',
            body: '라이브러리에 추가하기 가장 좋은 때는 프롬프트가 그대로 내보낼 만한 결과를 낸 바로 그 순간입니다. "나중에"는 결코 오지 않습니다.',
          },
          {
            name: '샘플 출력과 함께 브리프 보관하기',
            body: '미래의 당신은 무엇을 요청했는지뿐 아니라 "좋은 것"이 어땠는지 기억해야 합니다. 저장한 각 프롬프트 옆에 훌륭한 결과 하나를 함께 두세요.',
          },
          {
            name: '시스템이 아니라 문서 하나',
            body: '절대 관리하지 않을 태그 데이터베이스보다 고정된 메모 하나가 낫습니다. 문서가 붐빌 때에만 프로젝트나 폴더로 졸업하세요.',
          },
        ],
        takeaway:
          '저장된 브리프 세 개면 이미 시스템입니다. 대부분의 사람들의 AI 사용은 반복되는 다섯 가지 작업입니다 — 한 페이지짜리 라이브러리가 한 주의 대부분을 커버합니다.',
      },
    },
    // MT
    agenticcoding: {
      ...
      realSession: {
        title: '2. 실제 세션을 운전하기',
        intro:
          '같은 기능 — API에 속도 제한 추가 — 을 두 코딩 에이전트가 구현합니다. Claude Code와 Kiro CLI를 전환하고, 세션을 실행하고, 오른쪽에서 저장소가 바뀌는 것을 지켜보세요. 같은 결과, 두 가지 방법론.',
        workspaceTitle: 'claude-code — add rate limiting',
        terminalTitle: 'claude-code',
        stepNote:
          '각 턴을 단계별로 살펴보세요. Claude Code는 직접적인 도구 루프로 작동합니다 — 읽기, 편집, 테스트, 검증. Kiro는 스펙 우선으로 작동하여 코드를 건드리기 전에 .kiro/specs/에 요구사항, 설계, 작업을 작성합니다.',
        cliToggleLabel: 'CLI 선택: Claude Code 또는 Kiro',
      ...
      // MT
      spinUpTools: {
        title: '4. 코드 편집에서 도구를 빠르게 띄우기로',
        intro:
          '당신은 기존 저장소에서 에이전트를 운전해 보았습니다. 같은 에이전트는 제로-투-원에도 꼭 그만큼 능합니다 — 그리고 그것이 당신의 기본값을 조용히 바꿉니다. 작은 내부 도구, 스크립트, 대시보드의 비용은 무너졌고, 그래서 질문은 "이것이 만들 가치가 있는가?"에서 "거의 공짜라면 나는 무엇을 만들까?"로 옮겨 갑니다. 각 전환을 클릭해 보세요.',
        items: [
          {
            name: '제로-투-원, 편집만이 아니라',
            tagline: '그린필드가 이제 저렴하다',
            description:
              '에이전트는 한 단락에서 작동하는 앱을 스캐폴딩하는 것을, 오래된 것을 리팩터링하는 것만큼이나 쉽게 합니다. 당신이 없이 버텨온 내부 도구 — 로그 분류기, 온콜 대시보드, 데이터 가공 UI — 가 이제 분기가 아니라 한 오후입니다. 예전이라면 건너뛰었을 그것을 만드세요.',
          },
          {
            name: '스택을 알맞게 맞추기',
            tagline: '의식을 수명에 맞춰라',
            description:
              '팀 밖의 누구도 보지 않을 도구에는 당신의 프로덕션 프레임워크가 필요 없습니다. 단일 파일 앱, 스크립트, 또는 노코드/로우코드 빌더가 종종 완전한 프로젝트를 이깁니다. 무거운 스택은 사용자에게 출시되는 것을 위해 남겨 두고; 일회용 도구는 일회용이게 두세요.',
          },
          {
            name: '작은 도구는 작은 스펙이다',
            tagline: '같은 범위 설정 규율',
            description:
              '에이전트에게 도구를 의뢰하는 것은 지난 절의 습관을 그대로 씁니다: 명확한 결과, 중요한 제약, 완료의 정의. "이 로그들을 따라가며 스택 시그니처로 에러를 그룹화하고 상위 10개를 출력하는 CLI를 만들어"는 완전한 브리프입니다 — 한 번 범위를 정하고, 실행하게 두고, 결과를 검토하세요.',
          },
          {
            name: '노코드 경계선을 알라',
            tagline: '때로는 당신이 잘못된 빌더다',
            description:
              '모든 내부 도구가 당신이 유지보수하는 코드여야 하는 것은 아닙니다. 비기술 동료가 소유할 양식이나 일회성 UI라면, AI 앱 빌더는 그들이 스스로 바꿀 수 있는 무언가를 건네줍니다 — 풀 리퀘스트도, 루프 안의 당신도 없이. 언제 노코드로 넘길지 아는 것은 그 자체로 하나의 엔지니어링 판단입니다.',
          },
        ],
        takeaway:
          '당신의 저장소를 편집하는 에이전트는 처음부터 만드는 비용도 무너뜨립니다. 만들 가치가 있는 것에 대한 기준을 낮추고, 도구의 수명에 맞춰 스택을 알맞게 맞추고, 더 나은 답이 당신이 유지보수하는 코드 대신 동료가 소유하는 노코드 빌더일 때를 아세요.',
        selfExplainPrompt:
          '갖고 싶었지만 시간을 들일 가치가 없어 한 번도 만들지 않은 작은 내부 도구 하나를 대보세요. 이제 에이전트에게 건넬 한 단락 스펙은 무엇이며 — 당신이 소유하는 코드로 만들겠습니까, 아니면 동료가 소유하는 노코드 앱으로 만들겠습니까?',
        selfExplainAnswer:
          '예시: "우리 CI를 폴링하여 이번 주에 두 번 넘게 깜빡인(flaky) 테스트를 표시하는 대시보드. 스펙: CI API에서 최근 200회 실행을 읽고, 테스트 이름별로 실패를 그룹화하고, 비결정적 실패가 2회 이상인 테스트를 빈도순으로 나열하고, 로드 시 새로고침. 그것은 우리 CI 토큰을 건드리고 우리 저장소에 두고 싶으므로 내가 소유하는 단일 파일 앱으로 만들겠습니다 — 하지만 QA 리드가 각 깜빡임에 첨부하고 싶어 하는 분류 메모 양식은, 그들이 나 없이 필드를 바꿀 수 있도록 노코드 빌더로 넘기겠습니다."',
      },
      // MT
      stealThisSetup: {
        title: '5. 이 셋업을 훔쳐 가세요',
        intro:
          '코딩 에이전트와 싸우는 것과 그것으로 출시하는 것의 차이는 모델인 경우가 드뭅니다 — 셋업입니다. 이 스타터 AGENTS.md를 저장소 루트에 복사하고, 괄호를 채운 뒤, 아래 습관들을 훔쳐 가세요.',
        templateTitle: 'AGENTS.md 스타터 — 저장소 루트에 두세요',
        tipsTitle: '프로 팁',
        tips: [
          {
            name: '코드보다 계획을 먼저 요청하기',
            body: '"계획부터 말해줘 — 아직 코드는 쓰지 마." 5줄짜리 계획을 검토하는 것은 500줄짜리 diff를 검토하는 것보다 열 배 저렴합니다.',
          },
          {
            name: '커밋으로 체크포인트 만들기',
            body: '작동하는 단계마다 에이전트가 커밋하게 하세요. 잘못된 단계는 거대한 diff를 헤집는 고고학 발굴이 아니라 git revert 한 번이 됩니다.',
          },
          {
            name: '실제 오류를 붙여넣기',
            body: '있는 그대로의 스택 트레이스, 실제 실패 출력. "인증 관련해서 뭔가 던져"는 에이전트를 추측하게 만들지만, 트레이스는 그 줄로 데려갑니다.',
          },
          {
            name: '테스트를 지키기',
            body: '테스트가 실패하면 "테스트가 아니라 코드를 고쳐"라고 말하세요 — 압박받는 에이전트는 기꺼이 단언문을 고칠 것입니다. 핵심이 되는 동작을 명시하세요.',
          },
          {
            name: '한 작업, 한 세션',
            body: '길고 뒤섞인 세션은 오래된 컨텍스트를 쌓습니다. 끝내고, 커밋하고, 비우고, 새로 시작하세요 — AGENTS.md가 있으면 에이전트는 몇 초 만에 다시 적응합니다.',
          },
        ],
        takeaway:
          '좋은 AGENTS.md에 이 다섯 가지 습관을 더하면, 에이전트로 출시하는 팀과 일주일 만에 포기한 팀을 가르는 대부분이 갖춰집니다.',
      },
    },
    // MT
    agenticwork: {
      ...
      delegateInExcel: {
        title: '3. 엑셀에서 직접 보기',
        intro:
          '방금 추상적인 차원에서 위임을 차근차근 따라가 보았습니다. 여기 같은 종류의 작업을 수백만 명이 이미 사용하며 살아가는 도구 — 스프레드시트 — 에서 보여드립니다. Claude for Excel가 경비 시트를 출장 정책과 대조하는 것을 지켜보세요: 그것은 격자 안에서 작업하고, 규칙을 어기는 것을 표시하고, 정확한 셀을 인용합니다. 어떤 셀이든 클릭해 살펴보고, 실행을 눌러 Claude가 일하는 모습을 보세요.',
        ...
        flaggedLabel: '표시된 줄',
        prompt:
          '이 경비 시트를 우리 출장 정책과 대조해줘: 식사는 $75까지, 호텔은 $300까지. 금액 열을 통화 형식으로 표준화하고, 각 줄을 OK 또는 OVER로 표시하는 Status 열을 추가하고, 정책을 어기는 줄을 표시하고, 그 개수를 알려줘.',
        overwriteWarning: 'Claude가 D2:D12의 기존 값을 덮어씁니다.',
        messages: [
          '11개 줄 전부를 정책과 대조하겠습니다 — 식사는 $75까지, 호텔은 $300까지. 먼저 Status 열 [E1]을 추가합니다.',
          '금액 열을 통화 형식으로 표준화합니다. 이것은 D 열의 값을 다시 씁니다.',
          '정책을 준수하는 줄들을 OK로 표시합니다.',
          '다섯 줄이 한도를 초과합니다 — [D4], [D6], [D8], [D11], [D12]를 보세요. 그것들을 OVER로 표시하고 행을 표시합니다.',
          '데이터가 바뀌어도 정확하게 유지되도록, 표시된 줄의 개수를 [E13]에 수식으로 추가합니다.',
        ],
        takeaway:
          '무엇이 당신의 것으로 남았는지 주목하세요: 정책, 보낼지의 결정, 최종 검토. Claude는 모든 행에 걸쳐 읽기와 표시를 했고 그 작업을 셀 단위로 보여주었습니다 — 당신은 감독했습니다. 그것이 곧 위임입니다, 당신이 이미 쓰는 도구 안에서.',
        selfExplainPrompt:
          'Claude는 다섯 줄을 표시하고 정확한 셀(D4, D6, D8, D11, D12)을 인용했습니다. 단지 "5줄이 정책을 초과함"이라고 보고하는 것보다 구체적인 셀을 인용하는 것이 왜 더 중요할까요?',
        selfExplainAnswer:
          '인용은 당신이 믿어야만 하는 주장을 몇 초 만에 확인할 수 있는 주장으로 바꾸기 때문입니다. "5줄이 초과"는 Claude의 말을 그대로 받아들이라고 요구하지만, "D4, D6, D8, D11, D12를 보라"는 각각으로 바로 클릭해 들어가 금액을 규칙과 대조하고, 잘못된 판단이 재무팀에 도달하기 전에 잡아낼 수 있게 해줍니다. 추적 가능성은 결과 검토를 위임이 실제로 시간을 아낄 만큼 빠르게 만드는 요소입니다 — 그것이 없다면 당신은 240줄 전부를 직접 다시 확인하게 되는데, 그것이 바로 당신이 위임하려던 일입니다.',
      },
      guardrails: {
        title: '4. 가드레일을 설정하라',
        intro:
        ...
        demoTitle: '휴먼 인 더 루프, 실제로 보기',
        demoIntro:
          '가드레일은 직접 작동하는 걸 보기 전까지는 추상적으로 들립니다. 여기서 Claude Cowork이 폴더를 정리하다가 — 단 하나의 되돌릴 수 없는 단계, 즉 파일 삭제 직전에 멈춰서 당신의 허가를 구합니다. 보내기를 눌러 실행하세요; 선택하지 않으면 그 멈춤을 지나갈 수 없습니다.',
        demoSteps: [
          { content: '~/Invoices 폴더를 정리해 줘: 전부 YYYY-MM-거래처 형식으로 이름을 바꾸고, 중복 스캔본은 삭제해 줘.' },
          { label: '폴더를 읽음', content: '파일 48개를 찾았습니다. 41개는 일관된 명명 규칙이 없고, 7개는 이미 있는 송장의 중복 스캔본으로 보입니다.' },
          { label: '제자리에서 이름 변경', content: '41개 파일을 YYYY-MM-거래처 형식으로 변경했습니다. 데이터 손실 없음 — 이름 변경은 되돌릴 수 있습니다.' },
          {
            label: '삭제 전에 물어봄',
            content: '중복으로 식별한 7개 파일을 삭제할 준비가 됐습니다. 삭제는 되돌릴 수 없으므로, 건드리기 전에 당신의 승인이 필요합니다.',
            note: '이것이 휴먼 인 더 루프 가드레일입니다: 에이전트는 되돌릴 수 없는 작업을 준비해 대기시켰고 — 버튼은 사람이 누릅니다.',
          },
          { label: '승인 후에만 삭제', content: '승인됨 — 중복 7개를 제거했습니다. 폴더는 이제 깔끔하게 명명된 41개 파일, 중복 없음입니다.' },
        ],
        bridgeBlurb:
      ...
      // MT
      taskToTool: {
        title: '5. 작업이 도구가 되고 싶어 할 때',
        intro:
          '위임은 에이전트에게 일을 한 번 맡깁니다. 하지만 같은 일이 매주 돌아올 때, 예전에는 개발자와 예산이 필요했던 일을 이제 할 수 있습니다: AI에게 작은 도구 — 양식, 추적기, 계산기, 한 페이지짜리 앱 — 를 만들어 달라고 해서 당신과 팀이 재사용하는 것입니다. 기술은 방금 배운 그 브리핑과 같습니다; 결과물이 더 클 뿐입니다. 각 아이디어를 클릭해 보세요.',
        demoIntro:
          '먼저 신호를 알아채세요. 여기 Amazon Quick에서 같은 위임이 3주째 이어집니다 — 같은 브리프, 같은 단계, 같은 형태. 이것이 당신의 달력에서 반복되는 것을 보면, 그게 바로 신호입니다: 그 작업은 도구가 될 준비가 됐습니다.',
        demoSteps: [
          { content: '매주 월요일과 똑같이: 이번 주 예약 내보내기를 가져와 내일 예약 목록을 만들고, 중복 예약을 표시해 줘.' },
          { label: '익숙한 단계를 실행', content: '내보내기를 읽고, 시간순으로 정렬하고, 취소 건과 대조했습니다 — 지난 두 월요일과 동일한 루틴. 중복 예약 1건 표시됨.' },
          { label: '또다시 — 전달', content: '내일 목록이 준비됐습니다. 정확히 같은 이 작업이 3주 연속: 매번 ~10분, 같은 입력, 같은 출력 형태.' },
        ],
        items: [
          {
            name: '위임 대 의뢰',
            tagline: '한 번 끝낸 일 대 계속 갖는 도구',
            description:
              '위임은 작업 하나를 끝냅니다: "이번 달 경비를 대조해." 의뢰는 그것을 매달 해주는 것을 줍니다: "다음 달 시트를 떨궈 넣을 수 있는 경비 검사기를 만들어 줘." 위임이 계속 반복되면, 그것이 도구가 되고 싶어 한다는 신호입니다.',
          },
          {
            name: '이제 "노코드"가 실제로 의미하는 것',
            tagline: '만들지 말고 설명하라',
            description:
              '작은 내부 도구를 얻기 위해 더 이상 소프트웨어를 작성하거나 누군가를 고용할 필요가 없습니다. 원하는 것을 평범한 말로 설명하면 — 필드, 규칙, 출력 — AI 앱 빌더가 클릭할 수 있는 작동하는 앱을 만들어 냅니다. 작업을 브리핑하는 것과 똑같은 동작이며, 출력이 마침 소프트웨어일 뿐입니다.',
          },
          {
            name: '도구 모양의 작업 알아보기',
            tagline: '반복적 + 수작업 + 구조적',
            description:
              '작업이 도구가 되고 싶어 하는 것은 세 가지가 참일 때입니다: 그것을 거듭거듭 하고, 손으로 하기 번거롭고, 구조가 있습니다 — 매번 같은 필드와 단계. 다시 입력하는 접수 양식, 다시 만드는 주간 시트, 복사하고 손보는 체크리스트 — 그것들은 의뢰되기를 기다리는 도구이며, 클리닉이든 식당이든 학교 사무실이든 재무 백오피스든 마찬가지입니다.',
          },
          {
            name: '당신의 역할은 바뀌지 않는다',
            tagline: '여전히 지휘하고 검토한다',
            description:
              '도구를 의뢰하는 것은 작업을 위임하는 것과 같은 규율입니다: 명확한 브리프, 만든 것을 직접 써보는 체크포인트, 그리고 누군가 의지하기 전에 결과를 책임지는 당신. 당신은 프로그래머가 되는 것이 아니라 — 소프트웨어를 만들게 할 수 있는 사람이 됩니다. "이건 기술자들을 위한 것"이라는 두려움은 바로 여기서 끝납니다.',
          },
        ],
        walkthroughTitle: '매일의 노동에서 당신이 소유하는 도구로',
        stepLabel: '단계',
        steps: [
          {
            label: '반복되는 고통',
            content:
              '한 클리닉 접수처가 새 환자마다 종이 양식에서 세 개의 다른 화면으로 정보를 다시 입력하고, 그런 다음 짧은 요약을 손으로 씁니다. 환자당 20분, 일주일에 수십 번 — 매번 똑같은 필드.',
            note: '이것은 위임할 작업 하나가 아니라 영원히 반복됩니다. 그것이 단서입니다.',
          },
          {
            label: '모양을 알아본다',
            content:
              '반복적, 수작업, 구조적 — 같은 아홉 개 필드와 같은 요약을 거듭거듭. 그것이 도구 모양의 작업입니다. "이걸 누구에게 넘기지?"라고 생각하기를 멈추고 "무엇이 그냥 이걸 해줄 수 있을까?"라고 생각하기 시작합니다.',
            note: '알아보는 것이 기술입니다. 만드는 것은 이제 쉬운 부분입니다.',
          },
          {
            label: '도구를 설명한다',
            content:
              '"이 아홉 개 필드로 간단한 접수 양식을 만들어 줘. 전화번호와 ID 형식을 검증해. 내가 제출하면, 한 단락 요약과 우리 시트에 붙여넣을 수 있는 깔끔한 행을 보여줘. 모든 것을 페이지 안에 둬 — 데이터가 페이지를 떠나지 않게." 당신은 브리핑하는 것이지 코딩하는 것이 아닙니다.',
            note: '위임과 같은 네 부분 브리프입니다: 맥락, 원하는 것, 형식, 제약.',
          },
          {
            label: '만든 것을 검토한다',
            content:
              '몇 분 뒤 작동하는 양식이 있습니다. 실제(익명화된) 사례 하나로 시험해 보고, 잘못 읽힌 필드 레이블을 고치고, 요약을 두 문장 짧게 해달라고 합니다. 업데이트됩니다. 위임한 초안을 검토하는 바로 그 방식으로 출력을 검토했습니다.',
            note: '체크포인트는 변하지 않습니다 — 문서 대신 앱을 검토하게 될 뿐입니다.',
          },
          {
            label: '팀이 그것을 사용한다',
            content:
              '접수처는 이제 세 개의 화면에 다시 입력하는 대신 양식 하나를 채웁니다. 20분이 2분이 되었습니다. 개발자도 예산 요청도 없이 한 오후에 의뢰했고 — 내일 요청만으로 바꿀 수 있습니다.',
            note: '식당 주인은 공급업체 주문에, 학교 사무실은 동의서에 같은 일을 할 수 있습니다. 패턴은 옮겨 다닙니다.',
          },
        ],
        takeaway:
          '사다리에는 네 개의 가로대가 있습니다: AI를 잘 쓰고, 최고의 프롬프트를 저장하고, 작업 전체를 위임하고 — 작업이 계속 돌아올 때 — 그것을 대신 해주는 작은 도구를 의뢰하는 것. 각 가로대는 같은 기술(명확한 브리프와 실제 검토)이며, 더 큰 결과물을 향합니다.',
        selfExplainPrompt:
          '당신이나 팀이 매주 손으로 다시 하는, 매번 같은 모양인 작업 하나를 떠올려 보세요. 그것을 위해 AI에게 만들어 달라고 할 작은 도구를 평범한 말로 설명해 보세요 — 입력, 규칙이나 단계, 그리고 돌려받고 싶은 출력.',
        selfExplainAnswer:
          '예시: "매주 금요일 우리 식당은 그 주의 공급업체 배송을 주문서와 대조하여 부족분을 잡아냅니다. 도구: 주문 목록과 배송 전표를 붙여넣는 페이지; 그것을 한 줄씩 대조하고, 부족하거나 과다 청구된 것을 차액과 함께 표시하고, 한 줄 요약과 공급업체에 보낼 목록을 줍니다. 아무것도 페이지를 떠나지 않습니다." 입력, 명확한 규칙, 확인 가능한 출력 — 그것이 의뢰 준비가 된 브리프이며, 작업을 위임하기 위해 쓸 브리프와 똑같습니다. 결과 대신 도구를 요청할 뿐입니다.',
      },
      // MT
      briefLibrary: {
        title: '6. 훔쳐 쓸 수 있는 브리프',
        intro:
          '위임을 처음부터 발명할 필요는 없습니다. 이 브리프 템플릿을 복사하고, 괄호를 채우고, 실수가 실제로 비용이 될 지점에 체크포인트를 두세요.',
        templateTitle: '위임 브리프 — 괄호를 채우세요',
        exampleTitle: '같은 브리프, 채워 넣은 것',
        tipsTitle: '프로 팁',
        tips: [
          {
            name: '실수가 비싸지는 지점에 체크포인트 두기',
            body: '무언가 전송, 결제, 삭제, 게시되기 전 — 바로 그곳에 "먼저 보여줘"가 들어갑니다. 그 이전의 모든 것은 감독 없이 실행해도 됩니다.',
          },
          {
            name: '무엇이 필요한지 묻기',
            body: '브리프를 "이 브리프에서 빠진 것이 무엇이지?"로 끝맺으세요. 에이전트는 막히기 전에 자신의 걸림돌을 찾아내는 데 능합니다.',
          },
          {
            name: '완료를 정의하라, 아니면 결코 완료되지 않는다',
            body: '"내가 검토할 수 있는 표"가 "지출을 좀 살펴봐"보다 낫습니다. 결과물을 2분 안에 확인할 수 없다면, 아직 정의하지 않은 것입니다.',
          },
          {
            name: '에스컬레이션을 명시하기',
            body: '"규칙이 모호하면 결정하지 말고 \'사람 필요\' 아래에 나열해" — 자신만만한 오판을 막는 한 줄입니다.',
          },
        ],
        takeaway:
          '재사용 가능한 브리프는 위임을 글쓰기 연습에서 빈칸 채우기 습관으로 바꿉니다 — 그리고 체크포인트 줄이 그것을 안전하게 지키는 부분입니다.',
      },
    },
    // MT
    genaibeyondtext: {
