...
// Comparison array lives in the translation tree under modules.agents.mcpSection.
// EN values are the source of truth; SV/KO fall back to EN since legacy mCPSectionSv had `intro: ''` placeholder.
const COMPARISON = [
  { aspect: 'Scope', functionCalling: 'Per-model, per-integration', mcp: 'Universal protocol, any client' },
  { aspect: 'Schema format', functionCalling: 'Varies (OpenAI, Anthropic, etc.)', mcp: 'Standardized JSON-RPC' },
  { aspect: 'Tool reuse', functionCalling: 'Rewrite for each provider', mcp: 'Write once, use everywhere' },
  { aspect: 'Discovery', functionCalling: 'Manual — you pass schemas', mcp: 'Automatic — client queries server' },
  { aspect: 'Transport', functionCalling: 'HTTP API calls', mcp: 'stdio, SSE, HTTP (flexible)' },
  { aspect: 'Ecosystem', functionCalling: 'Vendor-specific', mcp: 'Open standard, growing ecosystem' },
]

export const MCPSection: React.FC = () => {
  const c = useTranslation().modules.agents.mcpSection
  return (
