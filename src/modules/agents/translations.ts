...
export const connectSv = {
  ...
  concepts: [
    ...
    { name: 'A2A — agenter pratar med agenter', analogy: 'Avdelningar som skickar förfrågningar till varandra', whatItIs: 'Medan MCP ansluter agenter till verktyg, ansluter A2A agenter till ANDRA agenter. Skapat av Google, stöds av 150+ organisationer.', example: 'Din supportagent upptäcker ett faktureringsproblem. Via A2A skickar den en återbetalningsförfrågan till ekonomiteamets agent.' },
  ],
...
export const connectKo = {
  ...
  concepts: [
    { name: 'MCP — 도구에 연결', analogy: '시스템을 위한 범용 접근 배지', whatItIs: '어떤 AI든 어떤 도구에든 연결할 수 있는 오픈 표준("AI의 USB-C"). 이미 2,000개 이상의 커넥터가 있습니다.', example: '회사가 티켓 시스템용 MCP 서버를 구축합니다. 이제 모든 AI 도구가 티켓을 생성하고 업데이트할 수 있습니다.' },
    ...
    { name: 'A2A — 에이전트끼리 대화', analogy: '부서 간 요청을 보내는 것', whatItIs: 'MCP가 에이전트를 도구에 연결하는 반면, A2A는 에이전트를 다른 에이전트에 연결합니다. Google이 만들고 150개 이상의 조직이 지원합니다.', example: '지원 에이전트가 청구 문제를 감지합니다. A2A를 통해 재무팀의 청구 에이전트에게 환불 요청을 보냅니다.' },
  ],
