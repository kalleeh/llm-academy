    ...
    <section aria-labelledby="quant-methods">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {mETHODST.map((m, i) => (
          ...
          >
            {m.name}
            {m.badge && <span className="ml-2 text-xs text-amber-400">{m.badge}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="rounded-lg border border-zinc-700 bg-zinc-900 p-5">
        <div className="mb-4">
          <h3 className="font-mono text-lg font-semibold text-zinc-100">{method.name}</h3>
          <p className="text-sm text-zinc-400">{method.tagline}</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-sm leading-relaxed text-zinc-300">{method.howItWorks}</p>
        </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3">
          ...
          <div className="rounded-md bg-red-500/10 p-3">
            ...
            <ul className="mt-2 space-y-1">
              {method.cons.map((c) => (
                <li key={c} className="text-sm text-zinc-300">• {c}</li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="rounded-md border border-zinc-700 bg-zinc-800/50 p-3">
          <span className="text-xs font-medium text-amber-400">When to use</span>
          <p className="mt-1 text-sm text-zinc-300">{method.whenToUse}</p>
        </div>
      </div>
    </section>
  )
