...
const ISSUES = [
  { label: 'Duplicates', detail: 'Acme Corp appears twice with slightly different names', color: 'text-red-400' },
  { label: 'Inconsistent format', detail: '"$48,000" vs "$48k" vs "2400" — same data, three formats', color: 'text-amber-400' },
  { label: 'Missing values', detail: 'Bolt has no email in one row, no status in another', color: 'text-amber-400' },
  { label: 'Inconsistent labels', detail: '"Cancelled" vs "Churned" — same meaning, different words', color: 'text-red-400' },
]
...
export const GarbageInOutBusiness: React.FC = () => {
  ...
  return (
    <section aria-labelledby="gigo-biz">
      <h2 id="gigo-biz" className="mb-4 font-mono text-xl font-bold text-zinc-100">{c.title}</h2>
      <p className="mb-2 max-w-2xl leading-relaxed text-zinc-300">
        The #1 rule of AI: <strong className="text-zinc-100">your AI is only as good as the data you feed it</strong>.
        Let&apos;s look at what messy data actually looks like — and why it matters.
      </p>

      {/* Visual: messy vs clean spreadsheet */}
      <div className="mb-6 rounded-lg border border-zinc-700 bg-zinc-900 overflow-hidden">
        <div className="flex items-center justify-between border-b border-zinc-700 bg-zinc-800 px-4 py-2">
          <span className="text-xs text-zinc-400">{showClean ? 'customers_cleaned.xlsx' : 'customers_raw.xlsx'} — Can you spot the problems?</span>
          <button onClick={toggleClean} className={`rounded px-3 py-1 text-xs transition-colors ${showClean ? 'bg-emerald-600/20 text-emerald-300' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'}`}>
                ...
                <tr key={i} className="border-b border-zinc-800/50 last:border-0">
                  {row.map((cell, j) => (
                    <td key={j} className={`px-3 py-2 whitespace-nowrap ${
                      !showClean && !cell ? 'bg-red-500/10 text-red-400 italic' :
                      !showClean && j === 1 && i > 0 && MESSY_ROWS[i-1]?.[1]?.toLowerCase().startsWith(cell.toLowerCase().slice(0, 4)) && cell !== MESSY_ROWS[i-1]?.[1] ? 'bg-amber-500/10 text-amber-300' :
                      'text-zinc-300'
                    }`}>
                      {cell || '(empty)'}
                    </td>
        ...
        {!showClean && (
          <div className="border-t border-zinc-700 bg-zinc-800/50 px-4 py-3">
        ...
        {showClean && (
          <div className="border-t border-zinc-700 bg-emerald-500/5 px-4 py-3">
      ...
      <InteractiveDemo
        ...
        steps={IMPACT_EXAMPLES.map((ex) => (
          <div key={ex.title} className={`rounded-lg border p-5 ${ex.color}`}>
            <p className="mb-3 text-sm font-semibold text-zinc-100">{ex.title}</p>
            <div className="space-y-2">
              <div className="rounded bg-zinc-800/50 p-3">
                <p className="text-xs font-medium text-zinc-500">The messy data</p>
                <p className="mt-1 text-sm text-zinc-300">{ex.messy}</p>
              </div>
              <div className="rounded bg-red-500/5 border border-red-500/20 p-3">
                <p className="text-xs font-medium text-red-400">What the AI learned</p>
                <p className="mt-1 text-sm text-zinc-300">{ex.result}</p>
              </div>
      ...
      <div className="mt-8 rounded-lg border border-zinc-700 bg-zinc-900 p-5">
        <p className="mb-3 text-sm font-medium text-zinc-100">{c.goodDataTitle}</p>
        <div className="grid gap-2 sm:grid-cols-2">
          {[
            ...
            <div key={q.label} className="rounded bg-zinc-800/50 px-3 py-2">
              <span className="text-xs font-medium text-amber-400">{q.label}: </span>
