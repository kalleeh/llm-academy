...
const SOURCES: DataSource[] = [
  ...
  {
    name: 'Academic', percent: 5, barColor: 'bg-purple-500', color: 'text-purple-400',
  ...
  {
    name: 'Wikipedia', percent: 3, barColor: 'bg-cyan-500', color: 'text-cyan-400',
  ...
  {
    name: 'Other', percent: 7, barColor: 'bg-rose-500', color: 'text-rose-400',
        ...
        {sOURCEST.map((s, i) => (
          <button key={s.name} onClick={() => toggle(i)}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="ml-auto font-mono text-sm text-zinc-400">{s.percent}%</span>
            </div>
            {selected === i && <p className="mt-2 text-xs leading-relaxed text-zinc-400">{s.details}</p>}
          </button>
        ))}
      </div>
    </section>
