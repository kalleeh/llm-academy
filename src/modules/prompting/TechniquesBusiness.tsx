...
export const TechniquesBusiness: React.FC = () => {
  ...
  <section aria-labelledby="techniques-biz">
    <h2 id="techniques-biz" className="mb-4 font-mono text-xl font-bold text-zinc-100">{c.title}</h2>
    ...
    <div className="mb-8 space-y-3">
      {c.techniques.map((t, i) => (
        <div key={t.name} className={`rounded-lg border p-5 ${(TECHNIQUES[i]?.color ?? "")}`}>
          <div className="mb-2"><span className="text-sm font-semibold text-zinc-100">{t.name}</span><span className="ml-2 text-xs text-zinc-400">— {t.analogy}</span></div>
          <div className="mb-2 rounded bg-zinc-800/50 px-3 py-2">
            <p className="text-xs text-zinc-500">Example prompt</p>
            <p className="mt-1 text-sm text-zinc-300 italic">{t.example}</p>
          </div>
          <p className="text-xs text-zinc-400">{t.why}</p>
        </div>
      ))}
    </div>

    <div className="mb-8 rounded-lg border border-zinc-700 bg-zinc-900 p-5">
      <p className="mb-3 text-sm font-medium text-zinc-100">{c.mistakesTitle}</p>
      <div className="space-y-2">
        {c.mistakes.map((m) => (
          <div key={m.mistake} className="flex items-start gap-3 rounded bg-zinc-800/50 px-4 py-3">
            <span className="text-xs text-red-400">✗</span>
            <div>
              <span className="text-sm text-zinc-300">{m.mistake}</span>
              <span className="mx-2 text-zinc-600">→</span>
              <span className="text-sm text-emerald-300">{m.fix}</span>
            </div>
          </div>
        ))}
      </div>
