...
export const CookbookBusiness: React.FC = () => {
  ...
  return (
    <section aria-labelledby="cookbook-biz">
      <h2 id="cookbook-biz" className="mb-4 font-mono text-xl font-bold text-zinc-900 dark:text-zinc-100">{c.title}</h2>
        ...
        {RECIPES.map((r, i) => (
          <div key={i} className={`rounded-lg border ${r.color}`}>
            <button onClick={() => toggle(i)} className="flex w-full items-center justify-between px-5 py-3 text-left" aria-expanded={expanded === i}>
              <div><span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{r.task}</span><span className="ml-2 rounded bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-xs text-zinc-600 dark:text-zinc-400">{r.category}</span></div>
            ...
            {expanded === i && (
              <div className="space-y-3 border-t border-zinc-200 dark:border-zinc-800 px-5 py-4">
                <div><p className="mb-1 text-xs font-medium text-zinc-500">{c.templateLabel}</p><pre className="rounded-lg bg-zinc-100 dark:bg-zinc-800 p-3 font-mono text-xs leading-relaxed text-zinc-700 dark:text-zinc-300 whitespace-pre-wrap">{r.template}</pre></div>
                <div><p className="mb-1 text-xs font-medium text-zinc-500">{c.resultLabel}</p><div className="rounded-lg border border-emerald-300 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/5 p-3"><p className="text-xs leading-relaxed text-zinc-700 dark:text-zinc-300 whitespace-pre-wrap">{r.example}</p></div></div>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
        <p className="mb-2 text-sm font-medium text-zinc-900 dark:text-zinc-100">{c.proTipTitle}</p>
        <p className="text-sm text-zinc-700 dark:text-zinc-300">{c.proTipText}</p>
      </div>
    </section>
  )
}
