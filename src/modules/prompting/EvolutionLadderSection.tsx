...
export const EvolutionLadderSection: React.FC = () => {
  ...
  return (
    <section aria-labelledby="evolution-ladder">
      <h2 id="evolution-ladder" className="mb-4 font-mono text-xl font-bold text-zinc-100">{c.title}</h2>
      <p className="mb-6 max-w-2xl leading-relaxed text-zinc-300">
        The same task — <strong className="text-zinc-100">&quot;{TASK}&quot;</strong> — gets
          ...
          {RUNGS.map((r, i) => (
            ...
            >
              <span className={`flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                i <= activeRung ? 'bg-violet-500/30 text-violet-300' : 'bg-zinc-800 text-zinc-500'
              }`}>
                {i + 1}
              </span>
              {r.name}
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="rounded-md bg-zinc-800 p-3">
              <p className="text-xs leading-relaxed text-zinc-300">
                <strong className="text-zinc-100">Why this level matters:</strong> {rung.why}
              </p>
            </div>
          </div>
        </div>
