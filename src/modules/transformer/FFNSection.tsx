...
function DenseView() {
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="rounded border border-zinc-300 dark:border-zinc-600 bg-zinc-100 dark:bg-zinc-800 px-6 py-2 text-xs text-zinc-700 dark:text-zinc-300">
      ...
      <div className="w-64 rounded-lg border border-orange-700 bg-orange-950 px-4 py-6 text-center">
        <div className="font-mono text-sm font-bold text-orange-700 dark:text-orange-300">Single FFN</div>
        <p className="mt-1 text-[10px] text-orange-700 dark:text-orange-400/70">
          All tokens processed by the same network — every parameter activated
        </p>
        <div className="mt-3 flex justify-center gap-1">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-8 w-2 rounded-sm bg-orange-200 dark:bg-orange-500/60" />
          ))}
        </div>
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <div className="rounded-lg border border-purple-700 bg-purple-950 px-4 py-2 text-center">
        <div className="font-mono text-xs font-bold text-purple-700 dark:text-purple-300">Router</div>
        <p className="text-[10px] text-purple-700 dark:text-purple-400/70">Picks top-K experts per token</p>
      </div>

      {/* Fan-out lines */}
      <svg width="280" height="24" className="text-zinc-500 dark:text-zinc-600">
            ...
            <div
              ...
              className={`w-16 rounded-lg border px-2 py-3 text-center transition-all ${
                active
                  ...
                  : 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 opacity-40'
              }`}
            >
              <div className={`text-[10px] font-bold ${active ? 'text-emerald-700 dark:text-emerald-300' : 'text-zinc-500'}`}>
                {name}
              </div>
              <div className="mt-1 flex justify-center gap-0.5">
                {Array.from({ length: 4 }).map((_, j) => (
                  <div
                    key={j}
                    className={`h-5 w-1.5 rounded-sm ${active ? 'bg-emerald-200 dark:bg-emerald-500/60' : 'bg-zinc-200 dark:bg-zinc-700'}`}
                  />
                ))}
              </div>
              {active && <div className="mt-1 text-[8px] text-emerald-700 dark:text-emerald-400">active</div>}
            </div>
          )
        })}
      </div>
  ...
  return (
    <section aria-labelledby="ffn-heading">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className={`rounded-lg border p-3 ${mode === 'dense' ? 'border-orange-700 bg-orange-950/50' : 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900'}`}>
          <h4 className="text-xs font-semibold text-orange-700 dark:text-orange-300">Dense FFN</h4>
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{c.intro}</p>
        </div>
        <div className={`rounded-lg border p-3 ${mode === 'moe' ? 'border-emerald-700 bg-emerald-950/50' : 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900'}`}>
          <h4 className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">Mixture of Experts</h4>
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{c.p5}</p>
        </div>
      </div>
    </section>
  )
