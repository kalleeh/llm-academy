...
export const ChoosingModelsBusiness: React.FC = () => {
  const c = useTranslation().modules.evaluation.choosing
  return (
  <section aria-labelledby="choosing-biz">
    <h2 id="choosing-biz" className="mb-4 font-mono text-xl font-bold text-zinc-900 dark:text-zinc-100">{c.title}</h2>
    ...
    <div className="mb-8 space-y-3">
      {TIERS.map((m) => (
        <div key={m.model} className={`rounded-lg border p-5 ${m.color}`}>
          <p className="mb-1 text-sm font-semibold text-zinc-900 dark:text-zinc-100">{m.model}</p>
          <p className="mb-2 text-xs text-zinc-500 italic">{m.analogy}</p>
          <div className="grid gap-2 sm:grid-cols-3">
            <div className="rounded bg-zinc-100 dark:bg-zinc-800/50 px-3 py-2"><span className="text-xs text-zinc-500">Best for: </span><span className="text-xs text-zinc-700 dark:text-zinc-300">{m.fit}</span></div>
            <div className="rounded bg-zinc-100 dark:bg-zinc-800/50 px-3 py-2"><span className="text-xs text-zinc-500">Cost: </span><span className="text-xs text-amber-300">{m.cost}</span></div>
            <div className="rounded bg-zinc-100 dark:bg-zinc-800/50 px-3 py-2"><span className="text-xs text-zinc-500">Speed: </span><span className="text-xs text-amber-300">{m.speed}</span></div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="mb-8 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
      <p className="mb-3 text-sm font-medium text-zinc-900 dark:text-zinc-100">{c.tipsTitle}</p>
      <div className="space-y-2">
        {TIPS.map((t) => (
          <div key={t.tip} className="rounded bg-zinc-100 dark:bg-zinc-800/50 px-4 py-3">
            <p className="text-sm text-zinc-800 dark:text-zinc-200">{t.tip}</p>
