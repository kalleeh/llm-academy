...
export const EmbeddingModelsSection: React.FC = () => {
  ...
  <section aria-labelledby="embedding-models">
    <h2 id="embedding-models" className="mb-4 font-mono text-xl font-bold text-zinc-900 dark:text-zinc-100">{c.title}</h2>
    <p className="mb-6 max-w-2xl leading-relaxed text-zinc-700 dark:text-zinc-300">
      {c.p2} <strong className="text-zinc-900 dark:text-zinc-100">generate text</strong> token by token; embedding
      models produce a <strong className="text-zinc-900 dark:text-zinc-100">fixed-size vector</strong> that represents
    ...
    <div className="mb-8 grid gap-4 sm:grid-cols-2">
      <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5">
        <h3 className="mb-2 font-mono text-sm font-semibold text-blue-700 dark:text-blue-400">LLM (Generative)</h3>
        <p className="mb-3 text-xs text-zinc-600 dark:text-zinc-400">{c.intro}</p>
      ...
      <div className="rounded-lg border border-violet-400 dark:border-violet-500/30 bg-violet-50 dark:bg-violet-500/5 p-5">
        <h3 className="mb-2 font-mono text-sm font-semibold text-violet-700 dark:text-violet-400">Embedding Model</h3>
        <p className="mb-3 text-xs text-zinc-600 dark:text-zinc-400">Input → Fixed-size vector</p>
        <div className="space-y-1.5 text-xs text-zinc-700 dark:text-zinc-300">
    ...
    <div className="mb-8 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
      <div className="border-b border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 px-4 py-3">
        <h3 className="font-mono text-sm font-semibold text-zinc-900 dark:text-zinc-100">{c.p5}</h3>
      </div>
          ...
          <thead>
            <tr className="border-b border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
              {['Model', 'Provider', 'Dims', 'Max Tokens', 'Quality', 'Cost/1M tokens', 'Notes'].map(h => (
                <th key={h} className="px-4 py-2 text-xs font-medium text-zinc-600 dark:text-zinc-400">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {MODELS.map((m, i) => (
              <tr key={m.name} className={`border-b border-zinc-200 dark:border-zinc-800 ${i === 0 ? 'bg-violet-50 dark:bg-violet-500/5' : ''}`}>
                <td className="px-4 py-2 font-mono text-xs text-zinc-800 dark:text-zinc-200">{m.name}</td>
                <td className="px-4 py-2 text-xs text-zinc-600 dark:text-zinc-400">{m.provider}</td>
                <td className="px-4 py-2 font-mono text-xs text-zinc-700 dark:text-zinc-300">{m.dims.toLocaleString()}</td>
                <td className="px-4 py-2 text-xs text-zinc-600 dark:text-zinc-400">{m.maxTokens}</td>
                <td className="px-4 py-2 text-xs text-amber-700 dark:text-amber-400">{m.perf}</td>
                <td className="px-4 py-2 font-mono text-xs text-green-700 dark:text-green-400">{m.cost}</td>
    ...
    <div className="rounded-md bg-zinc-100 dark:bg-zinc-800 p-4">
      <p className="text-xs leading-relaxed text-zinc-700 dark:text-zinc-300">
        <strong className="text-zinc-900 dark:text-zinc-100">Choosing a model:</strong> For production retrieval,{' '}
        <strong className="text-violet-700 dark:text-violet-400">Voyage-3-large</strong> leads benchmarks by 9-20% over
        competitors. For cost-sensitive workloads, OpenAI&apos;s{' '}
        <strong className="text-blue-700 dark:text-blue-400">text-embedding-3-large</strong> with reduced dimensions
        (1024) offers a good tradeoff. For self-hosted, <strong className="text-green-700 dark:text-green-400">BGE</strong>{' '}
        and <strong className="text-green-700 dark:text-green-400">GTE</strong> are the strongest open-source options.
      </p>
    </div>
  </section>
  )
